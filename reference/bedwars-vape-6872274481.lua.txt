				...
				bedwars.ProjectileController.calculateImportantLaunchValues = function(...)
					...
					if plr then
						...
						if calc and travelTime and (Horizontal.Value ~= 1 or Vertical.Value ~= 1) then
							calc = solve(plr, newlook.p, plr[TargetPart.Value].Position + (Vector3.new(velocity.X * (Horizontal.Value - 1), velocity.Y * (Vertical.Value - 1), velocity.Z * (Horizontal.Value - 1)) * travelTime), velocity, projSpeed, gravity, playerGravity) or calc
						end
