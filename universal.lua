		...
		if self.localprio < self:get(plr) or plr == lplr then
			...
			if self:getplayer(args[1], plr) then
				...
				for i, v in self.commands do
					if msg:sub(1, i:len() + 1):lower() == ';'..cmd:lower() then
						v(args, plr)
						return true
					end
				end
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			...
			for _, v in whitelist.data.WhitelistedUsers do
				if v.tags then
					for _, v in v.tags do
						v.color = Color3.fromRGB(unpack(v.color))
					end
				end
			end
	...
	local ESPLoop = {
		Drawing2D = function()
			...
			for i, v in Reference do
				...
				local facing = CFrame.lookAlong(rootPosition, cameraLook)
		...
		Drawing3D = function()
			...
			for i, v in Reference do
				if Distance.Enabled then
					local distance = localPosition and (localPosition - i.RootPart.Position).Magnitude or math.huge
					if distance < DistanceLimit.ValueMin or distance > DistanceLimit.ValueMax then
						for _, v in v do
							v.Visible = false
						end
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(runService.RenderStepped:Connect(function()
					...
					local facing = CFrame.lookAlong(entitylib.character.RootPart.Position, gameCamera.CFrame.LookVector * Vector3.new(1, 0, 1))
	
					for i, v in Reference do
						local dt = facing:PointToObjectSpace(i.RootPart.Position)
...
run(function()
	...
	local function Loop()
		...
		for i, v in Reference do
			local distance = Distance.Enabled and localPosition and (localPosition - i.RootPart.Position).Magnitude
			if distance then
				if distance < DistanceLimit.ValueMin or distance > DistanceLimit.ValueMax then
					v.Visible = false
					continue
...
run(function()
	...
	Sight = PromptChanger:CreateToggle({
		...
		Function = function(callback)
			for i, v in modified do
				i.RequiresLineOfSight = not callback and v.Sight
			end
		end,
