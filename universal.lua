...
Run(function()
	...
	local VisibilityParams: RaycastParams = RaycastParams.new()
	...
	local function IsEntityVisible(Ent): boolean
	    local Character: Model? = Ent.Character
	    local SelfCharacter: Model? = LocalPlayer.Character
	    local SelfRoot: BasePart? = Entity.character.RootPart
	    local TargetRoot: BasePart? = Ent.RootPart
	    if not Character or not SelfCharacter or not SelfRoot or not TargetRoot then
	        return false
	    end

	    local Cached = VisibilityCache[Character]
	    local Now: number = tick()
	    if Cached and Now < Cached.Expires and (Cached.SelfPosition - SelfRoot.Position).Magnitude < 0.5 and (Cached.TargetPosition - TargetRoot.Position).Magnitude < 0.5 then
	        return Cached.Visible
	    end

	    local Filter: {Instance} = {Camera, SelfCharacter, Character}
	    -- Match EntityLib's policy: players/NPCs between both characters are not
	    -- walls. Only actual world geometry may hide a target.
	    for _, Other in Entity.List do
	    ...
	    local SelfHead: BasePart? = SelfCharacter:FindFirstChild("Head") :: BasePart?
	    local Origins: {Vector3} = {SelfHead and SelfHead.Position or SelfRoot.Position, SelfRoot.Position}
	    local Points: {Vector3} = {}
	    local Seen: {[BasePart]: boolean} = {}
	    local function AddPart(Part: Instance?)
	        if not Part or not Part:IsA("BasePart") or Seen[Part] then
	            return
	        end
	        Seen[Part] = true
	        table.insert(Points, Part.Position)
	        -- Probe both visible edges of wider body parts. This lets a real small
	        -- opening count without ever accepting a completely covered model.
	        if Part.Name == "Head" or Part.Name:find("Torso") then
	            local Edge: Vector3 = Part.CFrame.RightVector * math.min(Part.Size.X * 0.4, 0.75)
	            table.insert(Points, Part.Position + Edge)
	            table.insert(Points, Part.Position - Edge)
	        end
	    end
	    for _, Name: string in VisibilityParts do
	        AddPart(Character:FindFirstChild(Name))
	    end
	    AddPart(TargetRoot)

	    local Visible: boolean = false
	    for _, Point: Vector3 in Points do
	        for _, Origin: Vector3 in Origins do
	            if not workspace:Raycast(Origin, Point - Origin, VisibilityParams) then
	                Visible = true
	                break
	            end
	        end
	    ...
	    VisibilityCache[Character] = {
	        Expires = Now + 0.075,
	        SelfPosition = SelfRoot.Position,
	...
	Killaura = vape.Categories.Blatant:CreateModule({
	        ...
	        if Callback then
	            repeat
	                ...
	                if not Mouse.Enabled or UserInputService:IsMouseButtonPressed(0) then
	                    Tool = GetTool()
	                    if Tool ~= LastTool or CachedInterest and not CachedInterest.Parent then
	                        LastTool = Tool
	                ...
	                local Attacked = {}
	                if Interest then
	                    -- EntityLib's single RootPart ray rejects targets whose torso is
	                    -- covered even when a head, hand, or leg is visible. Gather by
	                    -- range/team first, then apply the body visibility test below.
	                    ...
	                    if #Entities > 0 then
	                        ...
	                        for _, v: any in Entities do
	                            ...
	                            if FlatDelta.Magnitude > 0.001 and FacingMagnitude > 0.001 and LocalFacing.Unit:Dot(FlatDelta.Unit) < MinDot then
	                                continue
	                            end
	                            if Targets.Walls.Enabled and not IsEntityVisible(v) then
	                                continue
	                            end
	                            ...
	                            local ReachedLimit: boolean = #Attacked >= Max.Value
	    ...
	    Tooltip = "Attacks nearby targets; wall check accepts any genuinely visible body part, including legs through small openings"
	})
