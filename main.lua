...
local function finishLoading()
		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://api.catvape.dev/script?key=_key', true), 'init')(_scriptconfig)
				end
			]]
	...
	if not shared.vapereload then
		vape:CreateNotification('Finished Loading', (vape.VapeButton and 'Press the button in the top right' or 'Press '..table.concat(vape.Keybind or {'RightShift'}, ' + '):upper())..' to open GUI', 5)
...
vape, runPremium = loadstring(downloadFile('catsix/guis/'..gui..'.lua'), 'gui')(license)
shared.vape = vape
...
if not shared.VapeIndependent then
	...
	loadstring(downloadFile('catsix/libraries/authentication.lua'), 'authentication')(license)
	finishLoading()
else
