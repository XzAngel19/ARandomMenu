...
run(function()
	...
	calculatePath = function(target, blockpos, solidonly, breakmethod)
		...
		for i, v in exposed do
			...
			if magnitude <= 30 then
				table.insert(candidates, {distances[i], i, v, magnitude, magnitude > 0 and delta:Dot(look) / magnitude or 1})
			end
		end
			...
			bedwars.ClientDamageBlock:Get('DamageBlock'):CallServerAsync({
				...
				if result then
					...
					if anim then
						...
						task.wait(anim == 'Minimal' and 0.08 or 0.3)
						animation:Stop()
						animation:Destroy()
					end
				end
	...
	local KitModules = {
		...
		regent = {'AutoVoidRegent', 'VoidRegentAutoClutch', 'VoidRegentExtender'},
		...
		repeat
			...
			if names and #highlighted == 0 then
				...
				if #shown > 0 and not notified then
					...
					notif('Kits', `catvape supports {meta and meta.name or kit}\n{table.concat(shown, ', ')}`, 10, 'info')
				end
			end
			task.wait(1)
		until vape.Loaded == nil
		...
		Function = function(callback)
			if callback then
				...
				AutoTool:Clean(runService.Heartbeat:Connect(function()
					...
					if not (store.hand.tool and bedwars.ItemMeta[store.hand.tool.Name] or nil) ~= nil and (store.hand.tool and bedwars.ItemMeta[store.hand.tool.Name]).breakBlock ~= nil then
