		...
		Function = function(callback)
			...
			if callback then
				...
				bedwars.BlockSelector.getMouseInfo = function(...)
					...
					-- Solo se pisa el rango cuando el toggle esta prendido. Antes,
					-- apagarlo caia en un 24/18 hardcodeado que daba MAS alcance que
					-- prenderlo, y por eso el placing se sentia incomodo.
					if Select == 0 then
						if BlockReach.Enabled then Args.range = BlockRange.Value end
					elseif Select == 1 then
						if BreakReach.Enabled then Args.range = BreakRange.Value end
					end
			...
			if callback then
				...
				local function aimLaunchValues(...)
					...
					local function getLaunchValues(plr)
						...
						local velocity = projmeta.projectile == 'telepearl' and Vector3.zero or plr[TargetPart.Value].AssemblyLinearVelocity
						local newlook = CFrame.new(offsetpos, plr[TargetPart.Value].Position) * CFrame.new(projmeta.projectile == 'owl_projectile' and Vector3.zero or Vector3.new(bedwars.BowConstantsTable.RelX, bedwars.BowConstantsTable.RelY, bedwars.BowConstantsTable.RelZ))
						local calc, _, travelTime = solve(plr, newlook.p, plr[TargetPart.Value].Position, velocity, projSpeed, gravity, playerGravity)
						...
						if calc and travelTime and (Horizontal.Value ~= 1 or Vertical.Value ~= 1) then
							...
							local adjusted, _, adjustedTime = solve(plr, newlook.p, plr[TargetPart.Value].Position + lead, velocity, projSpeed, gravity, playerGravity)
							if adjusted then
...
run(function()
	...
	FPSBoost = vape.Legit:CreateModule({
		Name = 'FPS Boost',
		Category = 'Game',
		Icon = getvapeasset('catsix/assets/new/legit_fpsboost.png'),
		Function = function(callback)
			if callback then
...
run(function()
	-- Killaura especifico de BedWars: pega por el remote SwordHit (el unico que
	-- el servidor de BedWars acepta), no por touch como el universal.
	local Killaura
	local SwingRange, AttackRange, HorizontalRange, VerticalRange
	local AngleSlider, AirHit, SwingTime, AttackSpeed, SyncButton
	local nextAttack = 0
	...
	Killaura = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				repeat
					local sword = getSword()
					local tool = sword and sword.tool or store.hand.tool
					if tool and entitylib.isAlive and tick() >= nextAttack then
						local selfpos = entitylib.character.RootPart.Position
						...
						for _, v in plrs do
							...
							if facing.Magnitude > 0 and (delta * Vector3.new(1, 0, 1)).Magnitude > 0 then
								local angle = math.acos(math.clamp(facing.Unit:Dot((delta * Vector3.new(1, 0, 1)).Unit), -1, 1))
							...
							local reach = getReach(tool)
							local direction = CFrame.lookAt(selfpos, v.RootPart.Position).LookVector
							nextAttack = tick() + AttackSpeed.Value
							pcall(function()
								bedwars.SwordController.lastAttack = workspace:GetServerTimeNow()
								bedwars.Handler:Get('SwordHit'):Fire('SendToServer', {
									weapon = tool,
									...
									validate = {
										...
										targetPosition = {value = v.Character:GetPivot().Position},
										selfPosition = {value = selfpos + direction * math.max(delta.Magnitude - (reach - 0.001), 0)}
									}
								})
							end)
						end
		...
		Tooltip = 'BedWars killaura: pega por el remote SwordHit.'
	})
	SwingRange = Killaura:CreateSlider({Name = 'Swing range', Min = 1, Max = 30, Default = 25})
	AttackRange = Killaura:CreateSlider({Name = 'Attack range', Min = 1, Max = 30, Default = 16})
	HorizontalRange = Killaura:CreateSlider({Name = 'Horizontal range', Min = 1, Max = 30, Default = 28})
	VerticalRange = Killaura:CreateSlider({Name = 'Vertical range', Min = 1, Max = 30, Default = 28})
	...
	AirHit = Killaura:CreateTwoSlider({Name = 'Air hit chance', Min = 0, Max = 100, DefaultMin = 100, DefaultMax = 100})
	SwingTime = Killaura:CreateSlider({Name = 'Swing time', Min = 0.01, Max = 1, Default = 0.11, Decimal = 2})
	AttackSpeed = Killaura:CreateSlider({Name = 'Attack speed', Min = 0.05, Max = 1, Default = 0.285, Decimal = 3})
end)
